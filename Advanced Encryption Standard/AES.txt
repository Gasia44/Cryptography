...
//AES implementation 
//You just need to provide the desired plaintext and the key in a string format ex:("AB15....")
//and the algorithm will take care of everything and do encryption using AES, after all you will see 
//the key, the plaintext and the cyphertext printed 
//at the bottom in the main you can edit the plaintext and the key
//the name of the functions tell what the function does, for example "mixCol" function, do The Mix Column step... 
...
public class AES {
	 ...
	 */
	// public static final int[][] invsbox = {{0x52, 0x09, 0x6a, 0xd5, 0x30,
	...
	public static final int[][] mix_col_mat = { { 0x02, 0x03, 0x01, 0x01 },
			{ 0x01, 0x02, 0x03, 0x01 }, { 0x01, 0x01, 0x02, 0x03 },
			{ 0x03, 0x01, 0x01, 0x02 } };

	public static void subtitutionBoxRow(int[] g) {
		for (int i = 0; i < g.length; i++) {
			int hex = g[i];
			g[i] = sbox[hex / 16][hex % 16];
		}
	}

	public static void sBoxMatrix(int[][] state) {
	...
	public static int[][] addRoundKey(int[][] state, int[][] keymatrix) {
		int[][] result = new int[4][4];
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				result[i][j] = state[i][j] ^ keymatrix[i][j];
			}
		}
		return result;
	}

	public static int[] shiftRow(int[] g) {
		int[] temp = new int[4];
		temp[0] = g[1];
		temp[1] = g[2];
		temp[2] = g[3];
	...
	public static void shiftRowMatrix(int[][] state) {
		state[1] = shiftRow(state[1]);
		state[2] = shiftRow(shiftRow(state[2]));
		state[3] = shiftRow(shiftRow(shiftRow(state[3])));
	...
	public static int[][] mixCol(int[][] state) {
		int[][] result = new int[4][4];
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 4; j++)
				result[i][j] = 0;
		...
		for (int j = 0; j < 4; j++) {
			result[0][j] = fun2(state[0][j]) ^ fun3(state[1][j]) ^ state[2][j]
					^ state[3][j];
			result[1][j] = state[0][j] ^ fun2(state[1][j]) ^ fun3(state[2][j])
					^ state[3][j];
			result[2][j] = state[0][j] ^ state[1][j] ^ fun2(state[2][j])
					^ fun3(state[3][j]);
			result[3][j] = fun3(state[0][j]) ^ state[1][j] ^ state[2][j]
					^ fun2(state[3][j]);
		}
		return result;
	}
	...
	public static int[][] keyExpansion(int[][] key) {
		...
		for (int i = 4; i < 44; i++) {
			...
			if (i % 4 == 0) {
				temp = shiftRow(temp);
				subtitutionBoxRow(temp);
				temp[0] ^= tempRound;
				tempRound = fun2(tempRound);
	...
	public static void print(int[][] state) {
		for (int col = 0; col < state.length; col++) {
			for (int row = 0; row < state[col].length; row++) {
				System.out.print(Integer.toHexString(state[col][row]) + "   ");
			}
			System.out.println();
		}
		System.out.println();
	...
	public static void printInLine(int[][] state) {
		for (int col = 0; col < 4; col++) {
			for (int row = 0; row < 4; row++) {
				if (state[row][col] <= 15)
					System.out
							.print("0" + Integer.toHexString(state[row][col]));
				else
	...
	public static int[][] parseToArray(String text) {
		...
		for (int col = 0; col < 4; col++)
			for (int row = 0; row < 4; row++) {
				String temp_Text = Character.toString((text.charAt(i)))
						+ Character.toString(text.charAt(i + 1));
				res[row][col] = Integer.parseInt(temp_Text, 16);
				i = i + 2;
			}
		return res;
	...
	public static int[][] encrypt(int[][] state, int[][] key) {
		...
		result = addRoundKey(result, temp);

		for (int round = 1; round <= 10; round++) {
			sBoxMatrix(result);
			shiftRowMatrix(result);
			if (round != 10)
				result = mixCol(result);
			for (int i = 0; i < 4; i++)
				for (int j = 0; j < 4; j++)
					temp[i][j] = w[j + 4 * round][i];
			result = addRoundKey(result, temp);
		}
	...
	public static void main(String[] args) {
		...
		//Here you can edit the plaintext and the key
		String plainText = "C7A71C1B46261602EB1EE48FDA8155A4";
		String keyText = "E760AF6CD6748C48264694F7326D0335";

		System.out.println("The Key is:");
		System.out.println(keyText);
		System.out.println();

		int[][] key = parseToArray(keyText);
		int[][] state = parseToArray(plainText);
		...
		//print(state);			//If you would like to see the result columns mode then uncomment this part
